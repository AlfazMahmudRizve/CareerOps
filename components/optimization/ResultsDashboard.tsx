    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* Score Card */}
                <motion.div 
                    className="md:col-span-1 bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col items-center justify-center"
                ...
                >
                    ...
                    <motion.div 
                        className={`relative w-32 h-32 flex items-center justify-center rounded-full border-4 shadow-lg ${getColor(score)} ${getGlow(score)}`}
                ...
                {/* Feedback Card */}
                <motion.div 
                    className="md:col-span-2 bg-zinc-900 border border-zinc-800 p-6 rounded-xl relative"
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                    custom={1}
                >
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <motion.button 
                            onClick={async () => {
                                ...
                                alert('Viral sharing text copied to clipboard! Paste it into a LinkedIn post.');
                            }}
                            className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors absolute top-4 right-4"
                    ...
                    <p className="text-zinc-300 leading-relaxed pr-24">{data.feedback}</p>
                </motion.div>
            </div>
